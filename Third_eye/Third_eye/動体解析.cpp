...
#define DISTANCELIMIT   7
#define JUDGNUM    8
#define Radius    9
#define PROCESSZONE_X    10
#define PROCESSZONE_Y    11
#define PROCESSZONE_WIDTH    12
#define PROCESSZONE_HEIGHT    13
#define MAGNIFICATION    14
#define LINENUMLIMIT    15

//外部ファイルに人の発見場所（X,Y）と発見からたった処理数（要するにPeoplePointの情報）を保存して実行時に読み込むように
...
std::vector<PeoplePoint*>* PeoplePointLoad(std::vector<std::string> temp) {

    std::vector<PeoplePoint*>* result = new std::vector<PeoplePoint*>();
    result->clear();

    PeoplePoint* tempPeoplePoint;
    
    for (int i = 0; i < temp.size(); i+=3) {
        tempPeoplePoint = new PeoplePoint();
        tempPeoplePoint->point = cv::Point2f(std::stoi(temp[i]), std::stoi(temp[i + 1]));
        tempPeoplePoint->deleteNum = std::stoi(temp[i + 2]);

        result->push_back(tempPeoplePoint);
    }
...
int main(int argc, char *argv[]) {
    ...
    std::vector<std::string> config = CSVLoader("Third_eye_config.cg");
    if (config.empty()) {
    ...
    if (fileName.size() >= 2) {
        //背景画像を取得
        frm = cv::imread(fileName[0], 1);
        bg = cv::imread(fileName[1], 1);
    }
    

    if (frm.empty() || bg.empty()) {
        printf("Cannot read file\n");
        return -1;
    }
    ...
    temp = CSVLoader("PeoplePoint.txt");

    peoplePoint = PeoplePointLoad(temp);
    ...
    if (debugFlag == 0) {
        writing_file << peoplePoint->size() << std::endl;
    }
    else {
    ...
    filename = "PeoplePoint.txt";

    
    writing_file.open(filename, std::ios::out);

    for (int i = 0; i < peoplePoint->size(); i++) {
        writing_file << (*peoplePoint)[i]->point.x << "," << (*peoplePoint)[i]->point.y << "," << (*peoplePoint)[i]->deleteNum << std::endl;
    }
    ...
    if (debugFlag != 0) {
        ...
        cv::imwrite("data/debug/" + dateStr + ".png", frm);
